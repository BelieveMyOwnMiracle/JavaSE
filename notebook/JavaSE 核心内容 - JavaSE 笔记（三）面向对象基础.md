...
我们也==可以在类中添加代码块，代码块同样会在对象构造之前进行，在成员变量初始化之后执行==：
...
我们在一开始介绍了，我们实际上是将`.class`文件丢给JVM去执行的，而每一个`.class`文件其实就是我们编写的一个类，我们在Java中使用一个类之前，JVM并不会在一开始就去加载它，而是在需要时才会去加载（优化）一般遇到以下情况时才会会加载类（整个程序在执行过程中第一次进行以下行为时触发，后续不会重复加载类模板）：
...
![image-20220921120241184](https://s2.loli.net/2022/09/21/stOGnxaPirZvjLF.png)
...
可以看到它是属于`java.lang`这个包下的类，并且这个类也导入了很多其他包中的类在进行使用。那么，为什么我们在使用这个类时，没有导入呢？实际上==Java中会默认导入`java.lang`这个包下的所有类==，因此我们不需要手动指定。
...
**`protected` ≠ “所有子类对象都能直接在外部调用父类方法”；它的访问范围是 子类的代码内部**
...
如果某个类中存在静态方法或是静态变量，那么我们可以通过==静态导入的方式将其中的静态方法或是静态变量直接导入使用==，但是同样需要有访问权限的情况下才可以：
...
![image-20220921214944267](https://s2.loli.net/2022/09/21/hkapOYVHBrjy7UC.png)
...
注意，我们==如果不希望子类重写某个方法，我们可以在方法前添加`final`关键字，表示这个方法已经是最终形态==：
...
![image-20220921225234226](https://s2.loli.net/2022/09/21/zfhZ3YdFeCgJu89.png)
...
在Java 14中，官方重磅推出了一种全新的类型，叫做**记录类型**，它专用于一些保存不可变的数据的类。记录类需要使用`record`而非`class`关键字声明，我们可以在IDEA中创建一个记录类型：
...
**注意：** 类中是可以自由编写方法的，和普通class一样，但是有些特殊方法会自动生成，下面会提到。

接着，我们可以像使用普通类一样，创建这个记录类型的对象：

```java
TestData data = new TestData(1, "Hello");
```

乍一看好像没啥区别，别急，我们看看它内置了些什么方法：

![image-20250708222002003](https://s2.loli.net/2025/07/08/MXnHf92ACcOojDR.png)

可以看到，它本身直接自带了获取内部成员数据的同名方法。我们再来尝试打印一下：
...
![image-20250708222100663](https://s2.loli.net/2025/07/08/YpmUV7uZlwnezEr.png)
...
可以看到`equals`方法也自动重写了，并且是自动比较我们设置的成员。如果我们不需要它的默认实现，也可以直接对其方法进行手动重写：
...
所以这种类型就是专门为了承载数据的类而生的。此外，记录类型也可以像普通类那样去实现接口：
...
我们之前介绍过，类可以通过继承（extends关键字）来实现类的能力复用、扩展与增强。但有的时候，并不是所有的类我们都希望能够被继承。所以，我们需要对继承关系有一些限制的控制手段，而密封类的作用就是**限制类的继承**。
...
![image-20250708223238812](https://s2.loli.net/2025/07/08/VXtiNou3vjrCkw2.png)

![image-20250708223246820](https://s2.loli.net/2025/07/08/yPdlXTWUmztnhY1.png)
...
原文链接：https://www.itbaima.cn/zh-CN/document/eldst1fgrbdkmfs7